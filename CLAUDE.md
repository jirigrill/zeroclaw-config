...
- **.env**: Active credentials file — loaded into the zeroclaw process via `EnvironmentFile=` in the systemd unit. Vars here are available to shell commands the agent runs.
...
- `github.token`, `github.ideas_repo` — **config.toml only** (read by `github_push_file` custom tool, not from env)
